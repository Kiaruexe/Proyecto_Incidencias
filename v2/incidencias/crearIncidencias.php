...
if (isset($_POST["crear"]) && ($permiso != 'cliente' || $tieneEquipos)) {
    ...
    try {
        ...
        $stmt->execute([
            $fecha, $estado, $tecnicoAsignado, $observaciones, $TDesplazamiento, $TIntervencion,
            $tipoFinanciacion, $idUsuario, $numEquipo, $incidencia, $cp, $localidad, $provincia, $direccionDetalle
        ]);
...
<body>
  <form method="POST">
    ...
    <!-- Campo para ingresar la incidencia -->
    <p>
      <label>Incidencia:</label><br>
      <textarea name="incidencia" placeholder="Introduzca la incidencia" required><?= ($_GET['incidenciaPrev'] ?? ''); ?></textarea>
    </p>

    <?php if ($permiso == 'cliente'): ?>
      <!-- Para clientes, se muestran directamente los equipos y direcciones -->
      <?php if ($tieneEquipos): ?>
        <p>
          ...
          <select name="equipo" required>
            ...
            try {
                ...
                while ($eq = $queryEquipos->fetch()) {
                    echo "<option value='" . ($eq['numEquipo']) . "'>" . ($eq['numEquipo']) . "</option>";
                }
            } catch (PDOException $e) {
    ...
    <?php elseif ($permiso == 'recepcion' || $permiso == 'admin' || $permiso == 'jefetecnico'): ?>
      <?php if (!isset($_GET['clienteElegido'])): ?>
        ...
        <p>
          ...
          <span class="nuevo-cliente">
            <form action="../usuarios/crearUsuarios.php" method="get" target="_blank">
      ...
      <?php else:
              ...
              echo '<input type="hidden" name="cliente" value="' . ($clienteElegido) . '">';
      ...
      ?>
        <p>Cliente seleccionado: <?= ($clienteData['usuario']); ?></p>
        ...
        <p>
          ...
          <select name="equipo" required>
            ...
            try {
                ...
                while ($eq = $queryEquipos->fetch()) {
                    echo "<option value='" . ($eq['numEquipo']) . "' 
                           data-tipo='" . ($eq['tipoMantenimiento']) . "'
                           data-cp='" . ($eq['cp']) . "'
                           data-provincia='" . ($eq['provincia']) . "'
                           data-localidad='" . ($eq['localidad']) . "'
                           data-direccion='" . ($eq['direccion']) . "'>" 
                           . ($eq['numEquipo']) . " - " . ($eq['numEquipo']) . "</option>";
                }
            } catch (PDOException $e) {
                echo "<option value=''>Error cargando equipos</option>";
            }
  ...
  <script>
    ...
    // Actualiza los campos ocultos cuando se selecciona una dirección (para receptor, admin y jefeTecnico)
